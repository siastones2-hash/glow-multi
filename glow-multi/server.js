...
app.use((req, res, next) => {
  ...
  if (req.path.startsWith('/api/')) {
    return res.status(503).json({
      ok: false,
      suspended: true,
      error: '서비스가 일시 중단되었습니다. 잠시 후 다시 이용해 주세요.',
...
async function runMgmtFeeCycle() {
  ...
  for (const s of remindR.rows) {
    ...
    const superMsg =
      `⏰ <b>관리비 D-3</b>\n\n` +
      `🏷 <b>${s.name || ''}</b> (${s.domain || ''})\n` +
      `💵 ₩${fee.toLocaleString('ko-KR')} · 기한 ${due}`;
    await sendTelegramToSuper(superMsg).catch(() => null);
    if (s.tg_token && s.tg_chat) {
      await sendTelegramRaw(s.tg_token, s.tg_chat, adminMsg).catch(() => null);
    } else {
      // 사이트 TG 미설정 → 슈퍼에게 관리자용 상세도 함께
      await sendTelegramToSuper(adminMsg).catch(() => null);
    }
...
async function buildMemberOpsDigest() {
  ...
  if (quiet) {
    const text = `오늘도 최신 상품으로 점검·유지 중입니다. (${today})`;
    const payload = {
      today,
      totalActive,
      activeKr,
      ...
      quiet: true,
      headline: text,
      intro: '판매 목록은 매일 점검해 최신 상태로 유지합니다.',
    };
...
const SMMKINGS_CURATED_SEEDS = [
  ...
  // —— SEO·상위노출 (백링크/오프페이지 · 순위 보장 아님, 효과 설명 필수) ——
  {
    ...
    name: '한국 백링크 패키지 (5개) ⭐',
    description: '한국 고권위 사이트에 백링크 5개가 생성됩니다. 검색엔진이 사이트를 인지·신뢰하는 신호가 쌓여, 장기적으로 검색 노출에 도움이 됩니다. 신규·리뉴얼 사이트 SEO 기초 작업에 많이 씁니다. ※순위 1페이지 보장은 아닙니다. 수량1=패키지1회 · 웹사이트 URL'
  },
  {
    id: 'skseo2', pl: 'seo', package: true, api_id: '3939', cost: 14.00, min: 1, max: 1, refill: 0,
    name: 'Dofollow 백링크 + 기사 (GoogleAPIs)',
    description: 'dofollow 백링크 1개 + 기사(약 400자+)가 등록됩니다. 외부에서 내 사이트로 연결되는 SEO 기초 신호(피인용)를 바로 만들 수 있어, 처음 시작하는 분에게 부담 없는 입문용입니다. ※단독으로 상위노출을 보장하지 않습니다. 수량1=1건 · 웹사이트 URL'
  },
  {
    id: 'skseo3', pl: 'seo', package: true, api_id: '7148', cost: 112.50, min: 1, max: 1, refill: 0,
    name: 'High-DA Web 2.0 백링크 (30개)',
    description: 'DA(도메인 권위)가 높은 Web 2.0 성격 사이트에 백링크 약 30개가 생성됩니다. 링크 프로필이 두꺼워져 검색엔진이 사이트를 더 잘 인지·평가하는 데 쓰입니다. ※순위 급등 보장은 아니며, 과다 주문은 피하세요. 수량1=패키지1회 · 웹사이트 URL'
  },
  {
    id: 'skseo4', pl: 'seo', package: true, api_id: '6285', cost: 150.00, min: 1, max: 1, refill: 0,
    name: 'EDU 백링크 패키지 (5개 + 기사)',
    description: '교육(.edu) 성격 사이트 백링크 5개 + 기사(약 500자+)가 작업됩니다. 신뢰도 높은 도메인 링크로 SEO 신호를 강화할 때 쓰는 패키지입니다. ※업종 무관 · 순위 보장은 아닙니다. 수량1=패키지1회 · 웹사이트 URL'
  },
  {
    id: 'skseo5', pl: 'seo', package: true, targetMult: 1.3, api_id: '7147', cost: 250.00, min: 1, max: 1, refill: 0,
    name: '게스트포스트 백링크 (20개 + 기사)',
    description: '트래픽·DR이 높은 사이트에 게스트포스트형 백링크 약 20개 + 기사(약 500자+)가 작업됩니다. 외부 노출과 SEO 신호를 동시에 노리는 고품질 패키지로, 본격 백링크 캠페인에 적합합니다. ※순위·유입 보장은 아닙니다. 수량1=패키지1회 · 웹사이트 URL'
  },
  {
    id: 'skseo6', pl: 'seo', package: true, api_id: '7459', cost: 252.00, min: 1, max: 1, refill: 0,
    name: '유튜브 오프페이지 SEO (임베드·백링크)',
    description: '영상 임베드·클라우드/EDU/Web2.0 등 프리미엄 백링크 스택(약 16개)이 유튜브 영상에 연결됩니다. 영상 관련 외부 신호가 쌓여 유튜브·웹 검색에서 발견될 가능성을 높이는 데 씁니다. ※조회수·구독 급등 보장은 아닙니다. 수량1=패키지1회 · 유튜브 영상 URL'
  },
  {
    id: 'skg14', pl: 'instagram', api_id: '5009', cost: 12.00, min: 100, max: 100000, refill: 0,
    name: 'Instagram 좋아요 — 한국 상위노출용 ⭐',
    description: '한국 계정 기반 좋아요가 게시물에 들어갑니다. 초반 참여가 붙으면 인스타 알고리즘이 탐색·인기 쪽으로 더 넓게 보여줄 가능성이 커집니다. ※네이버·구글 검색 상위노출과는 무관합니다. 게시물·릴스 URL'
  },
];
...
function glowRateForTargetMultiple(supplierUsd, marginMult, targetMult = SMMKINGS_TARGET_MULT) {
